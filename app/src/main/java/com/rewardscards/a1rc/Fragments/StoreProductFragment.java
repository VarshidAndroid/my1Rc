...
public class StoreProductFragment extends Fragment {
    ...
    ArrayList<StoreProductModel.Datum> list = new ArrayList<>();
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.store_product_category_fragment, null);
        SharedPrefsUtils.setSharedPreferenceString(getContext(),"back","0.1");
        ...
        store_number  = SharedPrefsUtils.getSharedPreferenceString(getContext(),"store_number");

        startAnim();
        StoreProductParam storeProductParam = new StoreProductParam(store_group_id,store_category_id,store_number);
        WebApi webApi = ApiUtils.getClient().create(WebApi.class);

        recycle_view = (RecyclerView) view.findViewById(R.id.recycle_view);

        Call<StoreProductModel> call = webApi.fetch_adverts_store_product(storeProductParam);
        call.enqueue(new Callback<StoreProductModel>() {
            @Override
            public void onResponse(Call<StoreProductModel> call, Response<StoreProductModel> response) {
                ...
                if (response.body().getMessage().equalsIgnoreCase("Fetch Product Succesfully")){
                    ...
                    adapter = new StoreProductAdapter(getActivity(), list);
